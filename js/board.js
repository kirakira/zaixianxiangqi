(function(app) {
    ...
    app.board.redrawBoard = redrawBoard;
    app.board.removeAllChildren = removeAllChildren;
    app.board.makeMove = makeMove;
    app.board.pieceClicked = pieceClicked;
    ...
    function drawPiece(i, j, piece) {
        ...
        var outer = createCircle(i, j, 23, "piece-outer", "piece-outer-" + app.chess.toPosition(i, j));
        var inner = createCircle(i, j, 20, "piece-inner", "piece-inner-" + app.chess.toPosition(i, j));
        var t = createText(i, j, PIECE_TEXTS[piece], "piece-text", "piece-text-" + app.chess.toPosition(i, j));
        if (app.chess.isRedPiece(piece)) {
    ...
    function erasePiece(i, j) {
        erasePieceCoverIfAny(i, j);
        removeElement("piece-text-" + app.chess.toPosition(i, j));
        removeElement("piece-inner-" + app.chess.toPosition(i, j));
        removeElement("piece-outer-" + app.chess.toPosition(i, j));
        putPieceCover(i, j);
    }

    function putPieceCover(i, j) {
        var cover = createCircle(i, j, 23, "piece-cover", "piece-cover-" + app.chess.toPosition(i, j));
        cover.setAttribute("stroke-width", "0");
        cover.setAttribute("fill-opacity", "0");
        // TODO: refactor the approach here so that there is no need to specify name 'xiangqi'
        cover.setAttribute("onmousedown", "xiangqi.board.pieceClicked(" + i.toString() + ", " + j.toString() + ")");
        cover.setAttribute("touchend", "xiangqi.board.pieceClicked(" + i.toString() + ", " + j.toString() + ")");
        insertElement(cover);
        // FastClick.attach(cover);
    }

    function erasePieceCoverIfAny(i, j) {
        var id = "piece-cover-" + app.chess.toPosition(i, j);
        ...
        for (i = 0; i < 10; ++i)
            for (j = 0; j < 9; ++j)
                if (app.chess.board[i][j] != 0)
                    drawPiece(i, j, app.chess.board[i][j]);
    }
    ...
    function makeMove(i1, j1, i2, j2) {
        selected = false;
        if (app.chess.board[i2][j2] != 0)
            erasePiece(i2, j2);
        drawPiece(i2, j2, app.chess.board[i1][j1]);
        erasePiece(i1, j1);
        app.chess.move(i1, j1, i2, j2);
        eraseHighlights();
        putHighlight(i1, j1);
        putHighlight(i2, j2);
    }
    ...
    function pieceClicked(i, j) {
        ...
        if (app.chess.board[i][j] != 0 && 
            ...
            eraseHighlights();
            putHighlight(i, j);
        } else if (selected && app.chess.checkMove(select_i, select_j, i, j)) {
