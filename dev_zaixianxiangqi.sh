...
if cd web/ && make; then
  ...
  GOOGLE_CLOUD_PROJECT=zaixianxiangqi-test GOOGLE_APPLICATION_CREDENTIALS=${CREDENTIALS_FILE} PORT=8083 SERVE_STATIC_FILES=1 go run cmd/zaixianxiangqi/main.go
else
  cd ..
fi
